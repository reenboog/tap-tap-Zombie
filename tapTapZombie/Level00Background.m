...
NSString* resource(NSString *file)
{
    return [NSString stringWithFormat: @"levels/0/0/%@", file];
}


@interface Level00Background()
...
@implementation Level00Background
...
- (id) initWithNumberOfRoads: (int) num
{
    if(self = [super init])
    {
        ...
        // moon
        CCSprite *moon = [CCSprite spriteWithFile: resource(@"moon.png")];
        moon.position = ccp(204, 295);
        [self addChild: moon];
        
        CCSprite *moonLight = [CCSprite spriteWithFile: resource(@"moon.png")];
